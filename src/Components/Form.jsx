...
function Form() {
  ...
  const setRef = (key) => (el) => {
    if (el) inputRefssss.current[key] = el;
  };
  const autofocus = (key) => {
  ...
  const allFilled = (array) =>
    array.every((el) => {
      ...
      if (Array.isArray(el)) return el.length > 0 && allFilled(el);
      if (typeof el === "object") {
        const values = Object.values(el);
        if (values.length === 0) return false;
        return allFilled(values);
      }
      return true;
    });
          ...
          if (status === "OK") {
            ...
            if (result.status === "OK") {
              ...
              resolve({
                ...
                ore: { full: result.duration.text, ore: ore, mins: mins },
              });
            } else {
  ...
  const isValidDate = (dateStr) => {
    ...
    return !isNaN(input.getTime()) && input >= todayObj;
  };
  ...
  const book = async () => {
    ...
    if (!isDateTimeInFuture(date, time)) {
      toast_error(t("You can't select a past date/time!"));
      return;
    }
    ...
    if (allFilled(params_array) && isValidTime(time) && isValidDate(date)) {
      ...
      if (!isDrivable && option !== "hour") {
        toast_error(t("route_not_drivable"));
        return;
      }
    ...
    <>
      <section className="mainform" id="mainform">
        ...
        <div className="content">
          <h1 data-aos="fade-right">Book your ride</h1>
          <div
            className="input"
            data-aos="fade-right"
